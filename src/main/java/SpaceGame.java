...
import util.WavefrontMeshLoader;
...
public class SpaceGame {
	...
	private void createShip() throws IOException {
		...
		ship = loader.loadMesh("game/ship.obj.zip");
	...
	private void createAsteroid() throws IOException {
		WavefrontMeshLoader loader = new WavefrontMeshLoader();
		asteroid = loader.loadMesh("game/asteroid.obj.zip");
	...
	private void createSphere() throws IOException {
		WavefrontMeshLoader loader = new WavefrontMeshLoader();
		sphere = loader.loadMesh("game/sphere.obj.zip");
	}
	...
	private void createCubemapProgram() throws IOException {
		int vshader = createShader("game/cubemap.vs", GL_VERTEX_SHADER);
		int fshader = createShader("game/cubemap.fs", GL_FRAGMENT_SHADER);
	...
	private void createShipProgram() throws IOException {
		int vshader = createShader("game/ship.vs", GL_VERTEX_SHADER);
		int fshader = createShader("game/ship.fs", GL_FRAGMENT_SHADER);
		int program = createProgram(vshader, fshader);
		glUseProgram(program);
	...
	private void createShotProgram() throws IOException {
		int vshader = createShader("game/shot.vs", GL_VERTEX_SHADER);
		int fshader = createShader("game/shot.fs", GL_FRAGMENT_SHADER);
		int program = createProgram(vshader, fshader);
		glUseProgram(program);
